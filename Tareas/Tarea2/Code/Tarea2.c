...
pthread_mutex_t mutex; // MUTEX PARA BLOQUEAR VAR COND
pthread_mutex_t protect; // MUTEX PARA BLOQUEAR SC
...
int posicion_aleatoria_x(){ //FUNCION PARA SETEAR UN VALOR RANDOM AL MOVIMIENTO X
    srand(rdtsc());
...
void set_chale(){ // INICIALIZAR CHALE DE MANERA RANDOM
    int i,j,x,y;
...
void print_chale(){ // IMPRIMIR MATRIZ CHALE
    int x, y;
    for (x = 0; x < FILAS; ++x)
        for (y = 0; y < COLUMNAS; ++y)
            printf("%c%c", chale[x][y], (y == COLUMNAS-1) ? '\n' : ' ');
}
void persecusion(int i, int x, char clown){ //FUNCION CUANDO MODERADO SE ENCUENTRA CON PAYASOS, INDICANDO SI ESCAPA O NO
...
{   //FUNCION CON LA CUAL LOS THREADS MODERADOS REALIZAN SU BUSQUEDA DE LOS PAYASOS
    ...
    while(1){
        pthread_mutex_lock(&mutex);//BLOQUEO VARIABLE DE CONDICION
        if(its_your_turn != 1){
                pthread_cond_wait(&cond_gen4,&mutex); //ESPERO MI TURNO PARA EJECUTAR
        }
        pthread_mutex_lock(&protect); //BLOQUEO SC
            if(p == 1 && s == 1){   //TERMINO EJECUCION DANDO PASO AL SIGUIENTE THREAD Y SALGO
                its_your_turn = 2;
                pthread_cond_signal(&cond_gen); //TERMINO EJECUCION DANDO PASO AL SIGUIENTE THREAD Y SALGO
                pthread_mutex_unlock(&protect);
                pthread_mutex_unlock(&mutex);
                pthread_exit(NULL);
            }
            for(int i = 0; i < MODERADOS; ++i){ // MOVIMIENTO DE CADA MODERADO
                char clown;
                int x = movimiento(4);
                if(x == 0){
...
void* busqueda_flaites(void *arg){    //FUNCION CON LA CUAL LOS THREADS flaites REALIZAN SU BUSQUEDA DE LOS PAYASOS
    // sem_wait(bloqueo_moderados);        //SE UTILIZA LA FUNCION RAND() PARA ESCOJER LA DIRECCION DE MOVIMIENTO 0: ARRIBA, 1: DERECHA,                                            2:ABAJO, 3: IZQUIERDA
    while(1){
        pthread_mutex_lock(&mutex);  //BLOQUEO VARIABLE CONDICION
        if(its_your_turn != 2){
            pthread_cond_wait(&cond_gen,&mutex);
        }
            pthread_mutex_lock(&protect); //BLOQUEO SC
            if(p == 1 && s==1){ // TERMINO EJECUCION DADNO PASO AL SIGUIENTE THREAD
                its_your_turn = 3;
            ...
            for(int i = 0; i < FLAITES; ++i){ //MOVIMIENTO DE CADA FLAITE
                int x = movimiento(4);
                if(x == 0){
...
void* escape(void *arg){ // FUNCION DE MOVIMIENTO DE LOS PAYASOS
    while(1){
        pthread_mutex_lock(&mutex); // BLOQUEO VAR COND SEGUN CUAL THREAD ENTRA
        if(its_your_turn != 3 && (int)*(int*)arg == 0){
            pthread_cond_wait(&cond_gen2,&mutex);
        }
        else if(its_your_turn != 4 && (int)*(int*)arg == 1){
        ...
        int x = movimiento(4);
        switch((int)*(int*)arg){ // MOVIMIENTO DEL THREAD EN CUESTION, PIÑERA 0 Y SANDWICH 1
            case 0:
                if (p == 1){
                    break;
                }
